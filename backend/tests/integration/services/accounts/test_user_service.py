...
async def test_activate_user_success(
    ...
    await activation_token_repo.create(user.id, "token123", 1)
    await db.flush()
...
async def test_new_activation_token_success(db, user_service, faker):
    user_data = create_user_data(faker)
    user, _ = await user_service.register_user(**user_data)

    token = await user_service.new_activation_token(user.email)

    assert token is not None
    assert token.user_id == user.id
    assert (
        await user_service._activation_token_repo.get_by_token(token.token)
        is not None
    )
...
async def test_new_activation_token_user_already_active(
    db, user_service, user_factory
):
    user = await user_factory.create(db, is_active=True)

    with pytest.raises(ValueError):
        await user_service.new_activation_token(user.email)


async def test_new_activation_token_db_error(user_service, faker, mocker):
    user_data = create_user_data(faker)
    user, _ = await user_service.register_user(**user_data)

    mock = mocker.patch.object(
        user_service._db, "commit", side_effect=SQLAlchemyError
    )
...
async def test_reset_password_token_success(
    ...
    user = await user_factory.create(db)
    user_returned, token = await user_service.reset_password_token(user.email)
...
async def test_reset_password_token_db_error(
    db, user_service, user_factory, password_reset_token_repo, mocker
):
    mock = mocker.patch(
    ...
    user = await user_factory.create(db)
    with pytest.raises(SQLAlchemyError):
        await user_service.reset_password_token(user.email)

    mock.assert_called_once()
    assert await password_reset_token_repo.get_by_user(user.id) is None


async def test_reset_password_complete_success(
    db, user_service, user_factory, password_reset_token_repo
):
    user = await user_factory.create_with_tokens(db, token="token")
...
async def test_reset_password_complete_expired(
    ...
    user = await user_factory.create(db)
    await password_reset_token_repo.create(user.id, "token123", -1)
    await db.flush()

    with pytest.raises(exc.TokenExpiredError) as e:
        await user_service.reset_password_complete(
            user.email, "Test123!", "token123"
        )
    assert str(e.value) == (
...
async def test_reset_password_complete_db_error(
    ...
    user = await user_factory.create_with_tokens(db, token="token5")
    with pytest.raises(SQLAlchemyError):
        await user_service.reset_password_complete(
            user.email, "Test123!", "token5"
        )
